...
int keepalive(Client* c)
{
    ...
    if (expired(&c->ping_timer))
    {
        if (!c->ping_outstanding)
        {
            ...
            if (len > 0 && (rc = sendPacket(c, len, &timer)) == SUCCESS1) // send the ping packet
    ...
    switch (packet_type)
    {
        ...
        case PUBREC:
        {
            ...
            if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
                ...
                rc = FAILURE;
            else if ((rc = sendPacket(c, len, timer)) != SUCCESS1) // send the PUBREL packet
...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0)
        goto exit;
    if ((rc = sendPacket(c, len, &connect_timer)) != SUCCESS1)  // send the connect packet
        goto exit; // there was a problem
...
{ 
    ...
    if (len <= 0)
        goto exit;
    if ((rc = sendPacket(c, len, &timer)) != SUCCESS1) // send the subscribe packet
        goto exit;             // there was a problem
